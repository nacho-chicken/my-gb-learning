...
There are precious few useful things to go over. One obvious topic that has
waited until now is that of audio. It's pretty complex, but thankfully
the audio capabilities of the GB are the most well-documented part of the
entire system. People love those nostalgic bleeps and bloops. Because of
that, I'm not going to spend much, if any, time here explaining the
intricacies of GB audio hardware.

In this project, you can play a note by pressing the A button.
I would have spent a lot more time getting various things to work, but
the VM I'm running in doesn't let BGB enable audio and it's a pain
to debug something when you have to constantly transfer files to another
system that actually works properly.
*/
