...
As should be apparent by now, once you get past a certain ROM size, the maximum
addressable space can get to be a problem. You only have $8000 addressable
bytes of ROM at any one time. However, the section starting from $4000 to
$7FFF can be swapped at any time with another $4000-byte section of ROM.
Of course, the amount of banks available depends on how big the ROM is made
at compile time.

This example will just use the smallest possible size ROM with swappable banks
(that is to say, three swappable ROM banks) for simplicity's sake.
A sentence will be printed to the BG layer, with a part from each available
swappable bank.

Apparently, SRAM can also have swappable banks, but I'm not going to be
messing with that here. Should work identically, in any case.
*/
